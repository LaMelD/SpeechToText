"""파이프라인 2: diarize(sherpa-onnx) → 성문 매칭 → 턴 병합 → gpt-transcribe → 전사문.

    python -m stt_local.transcribe 회의.m4a --out out/0902회의/ [--participants "홍길동,김철수"] [--keywords "용어"]

keywords.txt가 있으면 자동으로 읽는다. --participants를 주면 회의에 없는 사람에게 이름이 붙는 오탐을 막는다.

out/speakers.json에서 이름을 고친 뒤 같은 명령을 다시 실행하면 STT 없이 전사문만 다시 만든다.
출력 스키마는 STT_LOCAL-pyannote와 동일하다 (결과 비교용).
"""
import argparse
import asyncio
import json
...
from stt_local.common import (DB_DIR, DEFAULT_THREADS, check_prereqs, db_load, diarize, filter_db,
                             fmt_ts, load_audio, load_keywords, match, merge_turns, slice_wav)

STT_MODEL = "gpt-transcribe"
PRICE_PER_MIN = 0.0045
PROMPT_CHARS = 300   # 직전 텍스트를 prompt로 넘길 길이
PROMPT_TURNS = 12    # 문맥으로 볼 직전 턴 수
UNKNOWN_MIN_SEC = 3.0  # 이보다 짧은 Unknown 턴은 번호 없이 [불명]. 실측: Unknown 턴 57개 중 68%가 3초 미만 파편
...
def render(turns: list[dict], texts: dict[int, str], speakers: dict[str, dict],
           unknown_min: float = UNKNOWN_MIN_SEC) -> tuple[str, list[dict]]:
    """같은 이름의 인접 턴을 한 줄로. 빈 텍스트는 건너뛰고, 없는 텍스트는 [전사 실패].
    unknown_min초 미만 Unknown 턴은 "불명"으로 접는다 — 2초짜리 추임새마다 Unknown-23 같은 번호가
    붙으면 회의록이 지저분해진다. 긴 Unknown은 실제 미등록 화자일 수 있으니 번호를 유지한다."""
    rows: list[dict] = []
    for i, t in enumerate(turns):
        ...
        if name.startswith("Unknown") and t["end"] - t["start"] < unknown_min:
            name = "불명"
...
def main() -> None:
    ...
    ap.add_argument("--cluster-threshold", type=float, default=0.8,
                    help="클러스터링 임계값. 작으면 화자가 많아진다")
    ap.add_argument("--min-duration-on", type=float, default=1.5,
                    help="이보다 짧은 발화 구간은 버린다. 낮추면 과분할, 높이면 추임새 손실")
    ap.add_argument("--min-duration-off", type=float, default=0.5)
    ap.add_argument("--threshold", type=float, default=0.60, help="성문 매칭 임계값")
    ap.add_argument("--threads", type=int, default=DEFAULT_THREADS)
    ap.add_argument("--clips", type=int, default=10, help="centroid 계산에 쓸 화자별 구간 수")
    ap.add_argument("--keywords", default="", help="쉼표로 구분한 용어. keywords.txt·매칭된 실명·참석자는 자동 추가")
    ap.add_argument("--participants", default="",
                    help="쉼표로 구분한 참석자 실명. 지정하면 성문 DB 후보를 이들로 제한 (불참자 오배정 차단)")
    ...
    if dj.exists() and cj.exists():
        ...
        if sj.exists():
            ...
            sj.unlink()
        segs, cent = diarize(wave, num_speakers=a.num_speakers,
                             cluster_threshold=a.cluster_threshold,
                             min_duration_on=a.min_duration_on,
                             min_duration_off=a.min_duration_off,
                             threads=a.threads, clips=a.clips)
        if not cent:
            sys.exit("화자를 찾지 못했습니다")
        # config를 함께 남긴다: 튜닝 값이 결과를 크게 바꾸므로 비교할 때 필요하다
        dj.write_text(json.dumps({
            "segments": segs, "labels": list(cent), "n_samples": len(wave),
            "config": {"num_speakers": a.num_speakers, "cluster_threshold": a.cluster_threshold,
                       "min_duration_on": a.min_duration_on, "min_duration_off": a.min_duration_off,
                       "clips": a.clips},
        }, ensure_ascii=False), encoding="utf-8")
        np.save(cj, np.stack(list(cent.values())))

    # 2. match (speakers.json이 있으면 사용자가 고친 것으로 보고 그대로 씀)
    participants = [p.strip() for p in a.participants.split(",") if p.strip()]
    if sj.exists():
        speakers = json.loads(sj.read_text(encoding="utf-8"))
        print("speakers.json 사용")
    else:
        speakers = match(cent, filter_db(db_load(a.db), participants), a.threshold)
    ...
    if todo:
        ...
        keywords = list(dict.fromkeys(load_keywords() + [k.strip() for k in a.keywords.split(",") if k.strip()]
                                      + names + participants))
