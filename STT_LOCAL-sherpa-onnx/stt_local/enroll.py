...
성문 벡터가 campplus 192dim이라 STT_LOCAL-pyannote의 voiceprints/(256dim)와 호환되지 않는다.
model.txt 가드가 섞임을 막으므로 이 프로젝트에서 새로 등록해야 한다.
"""
import argparse
import unicodedata
...
from stt_local.common import (DB_DIR, DEFAULT_THREADS, centroid_from_clips, check_prereqs, db_add,
                              diarize, load_audio, slice_wav)


def extract(audio: str, out: Path, num_speakers: int | None, clips: int, threads: int,
            min_duration_on: float, cluster_threshold: float) -> None:
    wave = load_audio(audio)
    # 등록용 centroid는 긴 구간에서 뽑아야 안정적이다. 클립 수만큼 centroid에도 쓴다.
    segs, cent = diarize(wave, num_speakers=num_speakers, cluster_threshold=cluster_threshold,
                         min_duration_on=min_duration_on, threads=threads, clips=clips)
    if not cent:
        print("화자를 찾지 못했습니다")
        return
...
def commit(src: Path, db: Path) -> None:
    ...
    for d in sorted(p for p in src.iterdir() if p.is_dir()):
        ...
        if name.startswith("SPEAKER_") or not (d / "centroid.npy").exists():
            continue
        marker = d / ".committed"   # 같은 폴더를 두 번 커밋하면 행이 중복된다. 마커로 막는다
        if marker.exists():
            print(f"건너뜀: {name} (이미 등록됨. 다시 넣으려면 {marker} 삭제)")
            continue
        clips = sorted(d.glob("*.wav"))   # 클립을 지웠으면 남은 것으로 재계산. 안 지웠으면 결과 동일
        vec = centroid_from_clips(clips) if clips else np.load(d / "centroid.npy")
        db_add(db, name, vec)
        marker.write_text(f"{db}\n", encoding="utf-8")
        n += 1
        print(f"추가: {name}")
...
def main() -> None:
    ...
    ap.add_argument("--clips", type=int, default=6, help="클러스터당 클립 수 (centroid에도 같이 쓴다)")
    ap.add_argument("--cluster-threshold", type=float, default=0.8)
    ap.add_argument("--min-duration-on", type=float, default=3.0,
                    help="등록은 긴 발화만 쓰는 게 안정적이라 기본값을 transcribe(1.5)보다 높게 둔다")
    ...
    if a.commit:
        ...
        extract(a.audio, a.out, a.num_speakers, a.clips, a.threads,
                a.min_duration_on, a.cluster_threshold)
    else:
